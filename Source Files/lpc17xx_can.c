...
/********************************************************************//**
 * @brief		Initialize and Config CAN peripheral
 * @param[in]	None
 * @return 		None
 *********************************************************************/
...
{
	uint32_t data;
	printf(LPC_UART0,"Message ID:     0x%x08\n\r",CAN_Msg->id);
	printf(LPC_UART0,"Message length: %d02 BYTES\n\r",CAN_Msg->len);

	printf(LPC_UART0,"Message type:   ");
	if(CAN_Msg->type==DATA_FRAME)
	{
		printf(LPC_UART0,"DATA FRAME \n\r");
	}
	else
		printf(LPC_UART0,"REMOTE FRAME \n\r");
	printf(LPC_UART0,"Message format: ");
	if(CAN_Msg->format==STD_ID_FORMAT)
	{
		printf(LPC_UART0,"STANDARD ID FRAME FORMAT\n\r");
	}
	else
		printf(LPC_UART0,"EXTENDED ID FRAME FORMAT\n\r");

	data = (CAN_Msg->dataA[0])|(CAN_Msg->dataA[1]<<8)|(CAN_Msg->dataA[2]<<16)|(CAN_Msg->dataA[3]<<24);
	printf(LPC_UART0,"Message dataA:  0x%x08\n\r",data);
	data = (CAN_Msg->dataB[0])|(CAN_Msg->dataB[1]<<8)|(CAN_Msg->dataB[2]<<16)|(CAN_Msg->dataB[3]<<24);
	printf(LPC_UART0,"Message dataB:  0x%x08\n\n\r",data);
}
