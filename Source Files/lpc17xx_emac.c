...
*//**
* @file		lpc17xx_emac.c
* @brief	Contains all functions support for EMAC firmware library on LPC17xx
* @version	1.0
* @date		21. Jan. 2014
...
void EMAC_Config(void)
{
	...
	printf(LPC_UART0,"Init EMAC module\n\r");
	printf(LPC_UART0,"MAC[1..6] addr: %x02-%x02-%x02-%x02-%x02-%x02 \n\r", \
	...
	while (EMAC_Init(&Emac_Config) == ERROR)
	{
		...
		printf(LPC_UART0,"Error during initializing EMAC, restart after a while\n\r");
		for (i = 0x100000; i; i--);
	}
	...
	printf(LPC_UART0,"Initialize EMAC complete\n\r");
}
...
/*	if (((id1 << 16) | (id2 & 0xFFF0)) == EMAC_DP83848C_ID)
		...
		case EMAC_MODE_AUTO:
			...
			for (tout = EMAC_PHY_RESP_TOUT; tout; tout--)
			{
				...
				if (regv & EMAC_PHY_BMSR_AUTO_DONE)
				{
					...
					printf(LPC_UART0,"Auto-negotiation has completed\n\r");
					break;
				}
				if (tout == 0){
					// Time out, return error
	...
	if (((id1 << 16) | (id2 & 0xFFF0)) == EMAC_KSZ8031_ID)
	{
		switch(ulPHYMode)
		{
		case EMAC_MODE_AUTO:
...
int32_t EMAC_UpdatePHYStatus(void)
{
	...
	if (regv & EMAC_PHY_SR_DUP)
	{
		...
		printf(LPC_UART0,"Link is Full Duplex\n\r");
	...
	{
		/* Half duplex mode. */
		printf(LPC_UART0,"Link is Half Duplex\n\r");
		LPC_EMAC->IPGT = EMAC_IPGT_HALF_DUP;
	}
	if (regv & EMAC_PHY_SR_SPEED)
	{
	    /* 10MBit mode. */
		printf(LPC_UART0,"Link is 10MBit\n\r");
		LPC_EMAC->SUPP = 0;
	}
	else
	{
		/* 100MBit mode. */
		printf(LPC_UART0,"Link is 100MBit\n\r");
