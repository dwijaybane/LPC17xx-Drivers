...
{
	int16_t n=0,encoder0Pos=0,encoder0PinALast=0;

	n = GPIO_GetIntStatus(2, 8, 0);
	if ((encoder0PinALast == 0) && (n == 1))
	{
		GPIO_ClearInt(2,_BIT(8));
		if(GPIO_GetIntStatus(2,9,0) == 0)
		{
			encoder0Pos--;
		}
		else
		{
			GPIO_ClearInt(2,_BIT(9));
			encoder0Pos++;
		}
		printf(LPC_UART0,"\x1b[1;1HEncoder Position: %d04",encoder0Pos);
	}
	encoder0PinALast = n;
}
#endif
 ...
 * @brief		Read Current state on port pin that have input direction of GPIO
 * @param[in]	portNum		Port number to read value, in range from 0 to 4
 * @param[in]	Pin number, should be: 0..30
 * @return		Current value of GPIO port pin.
 *
 * Note: Return value contain state of port pin (bit) on that GPIO regardless
 * its direction is input or output.
 **********************************************************************/
uint8_t GPIO_Read_PinValue(uint8_t portnum,uint32_t pinvalue)
{
	uint8_t pin;
	uint32_t temp,portvalue;
	portvalue=GPIO_ReadValue(portnum);
	temp = portvalue & (1<<pinvalue);
	pin = temp>>pinvalue;
	return pin;
}
