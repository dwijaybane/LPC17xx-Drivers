...
void QEI_IRQHandler(void)
{
	...
	if (QEI_GetIntStatus(LPC_QEI, QEI_INTFLAG_DIR_Int) == SET)
	{
		...
		printf(LPC_UART0,(QEI_GetStatus(LPC_QEI, QEI_STATUS_DIR) == SET) ? "1" : "0");
		// Reset Interrupt flag pending
...
void QEI_Config (void)
{
	...
	// QEI Reload Configuration structure variable
	QEI_RELOADCFG_Type ReloadConfig;
	// Pin configuration for QEI
	...
	PinCfg.Portnum = 1;
	PinCfg.Pinnum = 20;
	PINSEL_ConfigPin(&PinCfg);   // MCI0
	PinCfg.Pinnum = 23;
	PINSEL_ConfigPin(&PinCfg);   // MCI1
	PinCfg.Pinnum = 24;
	PINSEL_ConfigPin(&PinCfg);   // MCI2
